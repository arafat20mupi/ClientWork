...
const AdminSignCopy = () => {
  const [SignCopy, setSignCopy] = useState([]);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get("/api/SignCopy");
        setSignCopy(response.data.servers);
      } catch (error) {
  ...
  const handleCancel = async (id) => {
    ...
    try {
      const response = await axios.put(`/api/SignCopyCancel/${id}`, { feedback });
